...
uint32_t countTag(const std::string& buff_s, const char * tag)
{
    const std::string tag_s(tag);
    if (tag_s.length() == 0) return 0;
    uint32_t count = 0;
    for (int32_t offset = buff_s.find(tag_s);
         offset != (int32_t)std::string::npos;
         offset = buff_s.find(tag_s, offset + tag_s.length()))
    {
        ++count;
    }
    return count;
...
int32_t sky_encode_req_xml(char *buff, int32_t bufflen, struct location_req_t *creq) {
 ...
 (36 + 18 + 28 + 30 + 18 + 28 + 24 + 18 + 18 + 12 + 12 + 8 + 8
    ...
    if (creq->payload_type == LOCATION_RQ_ADDR)
    ...
    for (i = 0; i < creq->cell_count; i++) {
        ...
        case DATA_TYPE_GSM:
            ...
            strncpy(p, gsm, sz);
            p += sz;
            p += sprintf(p, mcc, creq->gsm[i].mcc);
            p += sprintf(p, mnc, creq->gsm[i].mnc);
            p += sprintf(p, lac, creq->gsm[i].lac);
            p += sprintf(p, ci, creq->gsm[i].ci);
            p += sprintf(p, rssi, creq->gsm[i].rssi);
            p += sprintf(p, age, creq->gsm[i].age);
            sz = strlen(gsm_eof);
        ...
        case DATA_TYPE_CDMA:
            ...
            strncpy(p, cdma, sz);
            p += sz;
            p += sprintf(p, cdma_lat, creq->cdma[i].lat);
            p += sprintf(p, cdma_lon, creq->cdma[i].lon);
            p += sprintf(p, sid, creq->cdma[i].sid);
            p += sprintf(p, bsid, creq->cdma[i].bsid);
            p += sprintf(p, nid, creq->cdma[i].nid);
            p += sprintf(p, rssi, creq->cdma[i].rssi);
            p += sprintf(p, age, creq->cdma[i].age);
            sz = strlen(cdma_eof);
        ...
        case DATA_TYPE_UMTS:
            ...
            strncpy(p, umts, sz);
            p += sz;
            p += sprintf(p, mcc, creq->umts[i].mcc);
            p += sprintf(p, mnc, creq->umts[i].mnc);
            p += sprintf(p, lac, creq->umts[i].lac);
            p += sprintf(p, ci, creq->umts[i].ci);
            p += sprintf(p, rssi, creq->umts[i].rssi);
            p += sprintf(p, age, creq->umts[i].age);
            sz = strlen(umts_eof);
        ...
        case DATA_TYPE_LTE:
            ...
            strncpy(p, lte, sz);
            p += sz;
            p += sprintf(p, mcc, creq->lte[i].mcc);
            p += sprintf(p, mnc, creq->lte[i].mnc);
            p += sprintf(p, eucid, creq->lte[i].eucid);
            p += sprintf(p, rssi, creq->lte[i].rssi);
            p += sprintf(p, age, creq->lte[i].age);
            sz = strlen(lte_eof);
...
int32_t sky_decode_resp_xml(char *buff, int32_t buff_len, int32_t data_len,
    ...
    cresp->timestamp = 0;
    cresp->protocol = 0;
    cresp->payload_type = 0;

    memset(&cresp->location_ex, 0, sizeof(cresp->location_ex)); // zero out the counts
    ...
    if (strstr(buff, error) != NULL && strstr(buff, errorf) != NULL) {
        cresp->payload_type = LOCATION_RQ_ERROR;
        return 0;
    }

    cresp->payload_type = LOCATION_RQ;
    ...
    if (p != NULL && sscanf(p, distpoints, &fval) == 1)
        cresp->location_ex.distance_to_point = fval;

    if ((slen = get_xval(buff, street, streetf, &p)) > 0) {
        cresp->location_ex.street_num_len = slen;
        cresp->location_ex.street_num = p;
    }

    if ((slen = get_xval(buff, addr, addrf, &p)) > 0) {
        cresp->location_ex.address_len = slen;
        cresp->location_ex.address = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    if ((slen = get_xval(buff, city, cityf, &p)) > 0) {
        cresp->location_ex.city_len = slen;
        cresp->location_ex.city = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    if ((slen = get_xval(buff, metro1, metro1f, &p)) > 0) {
        cresp->location_ex.metro1_len = slen;
        cresp->location_ex.metro1 = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    if ((slen = get_xval(buff, metro2, metro2f, &p)) > 0) {
        cresp->location_ex.metro2_len = slen;
        cresp->location_ex.metro2 = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    if ((slen = get_xval(buff, postcode, postcodef, &p)) > 0) {
        cresp->location_ex.postal_code_len = slen;
        cresp->location_ex.postal_code = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    if ((slen = get_xval(buff, county, countyf, &p)) > 0) {
        cresp->location_ex.county_len = slen;
        cresp->location_ex.county = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    if ((slen = get_xval(buff, statec, closebr, &p)) > 0) {
        cresp->location_ex.state_code_len = slen;
        cresp->location_ex.state_code = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    // has to be right after the statec, reusing the pointer
    if ((slen = get_xval(p, closebr, statef, &p)) > 0) {
        cresp->location_ex.state_len = slen;
        cresp->location_ex.state = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    if ((slen = get_xval(buff, countryc, closebr, &p)) > 0) {
        cresp->location_ex.country_code_len = slen;
        cresp->location_ex.country_code = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    // has to be right after the countryf, reusing the pointer
    if ((slen = get_xval(p, closebr, countryf, &p)) > 0) {
        cresp->location_ex.country_len = slen;
        cresp->location_ex.country = p;
        cresp->payload_type = LOCATION_RQ_ADDR;
    }

    if ((slen = get_xval(buff, ip, ipf, &p)) > 0) {
        int32_t res;

        if (slen > 16) // 192.168.155.234
                {
            res = hex2bin(p, slen, cresp->location_ex.ip_addr, 16);

            if (res < 16) {
                printf("ip is too short\n");
            } else
                cresp->location_ex.ip_type = DATA_TYPE_IPV6;
        } else {
            res = hex2bin(p, slen, cresp->location_ex.ip_addr, 4);
...
char api_req_decode_ap(int32_t count, int32_t slen,
        struct location_req_t* req, char* buff) {
...
char* api_req_decode_cdma(int32_t count, struct location_req_t* req,
        char* buff, char& err_cdma) {
    req->cell_type = DATA_TYPE_CDMA;
    req->cdma = (struct cdma_t*) (calloc(count, sizeof(struct cdma_t)));
    char* p = buff;
    char * ps, * pe;
    while ((p = strstr(p, XML_TAG_CDMA)) != NULL) {
        ...
        if (p != NULL && p < pe && sscanf(p, XML_TAG_RSSIS, &dval) == 1) {
            ...
            req->cdma[req->cell_count].rssi = (int8_t) (dval);
        } else
            err_cdma++;
        ...
        uint32_t uval;
        if (p != NULL && p < pe && sscanf(p, XML_TAG_AGES, &uval) == 1)
            req->cdma[req->cell_count].age = uval;
...
char* api_req_decode_gsm(int32_t count, struct location_req_t* req,
        char* buff, char& err_gsm) {
    req->cell_type = DATA_TYPE_GSM;
    req->gsm = (struct gsm_t*) (calloc(count, sizeof(struct gsm_t)));
    char* p = buff;
    char * ps, * pe;
    while ((p = strstr(p, XML_TAG_GSM)) != NULL) {
        ...
        if (p != NULL && p < pe && sscanf(p, XML_TAG_CIS, &uval) == 1)
            req->gsm[req->cell_count].ci = uval;
        else
            err_gsm++;
        ...
        if (p != NULL && p < pe && sscanf(p, XML_TAG_RSSIS, &dval) == 1) {
            ...
            req->gsm[req->cell_count].rssi = (int8_t) (dval);
        } else
            err_gsm++;

        p = strstr(ps, XML_TAG_AGE);
        if (p != NULL && p < pe && sscanf(p, XML_TAG_AGES, &uval) == 1)
            req->gsm[req->cell_count].age = uval;
...
char* api_req_decode_lte(int32_t count, struct location_req_t* req, char* buff, char& err_lte) {
    req->cell_type = DATA_TYPE_LTE;
    req->lte = (struct lte_t*) (calloc(count, sizeof(struct lte_t)));
    char* p = buff;
    char * ps, * pe;
    while ((p = strstr(p, XML_TAG_LTE)) != NULL) {
        ...
        int32_t dval;
        if (p != NULL && p < pe && sscanf(p, XML_TAG_MCCS, &dval) == 1)
            req->lte[req->cell_count].mcc = (uint16_t) (dval);
        else
            err_lte++;

        p = strstr(ps, XML_TAG_MNC);
        if (p != NULL && p < pe && sscanf(p, XML_TAG_MNCS, &dval) == 1)
            req->lte[req->cell_count].mnc = (uint16_t) (dval);
        else
            err_lte++;
        ...
        if (p != NULL && p < pe && sscanf(p, XML_TAG_EUCIDS, &uval) == 1)
            req->lte[req->cell_count].eucid = uval;
        else
            err_lte++;

        p = strstr(ps, XML_TAG_AGE);
        if (p != NULL && p < pe && sscanf(p, XML_TAG_AGES, &uval) == 1)
            req->lte[req->cell_count].age = uval;
        ...
        if (p != NULL && p < pe && sscanf(p, XML_TAG_RSSIS, &dval) == 1) {
            ...
            req->lte[req->cell_count].rssi = (int8_t) (dval);
        } else
            err_lte++;
...
char* api_req_decode_umts(int32_t count,
        struct location_req_t* req, char* buff, char& err_umts) {
    req->cell_type = DATA_TYPE_UMTS;
    req->umts = (struct umts_t*) (calloc(count, sizeof(struct umts_t)));
    char* p = buff;
    char * ps, * pe;
    while ((p = strstr(p, XML_TAG_UMTS)) != NULL) {
        ...
        int32_t dval;
        if (p != NULL && p < pe && sscanf(p, XML_TAG_MCCS, &dval) == 1)
            req->umts[req->cell_count].mcc = (uint16_t) (dval);
        else
            err_umts++;

        p = strstr(ps, XML_TAG_MNC);
        if (p != NULL && p < pe && sscanf(p, XML_TAG_MNCS, &dval) == 1)
            req->umts[req->cell_count].mnc = (uint16_t) (dval);
        else
            err_umts++;

        p = strstr(ps, XML_TAG_LAC);
        if (p != NULL && p < pe && sscanf(p, XML_TAG_LACS, &dval) == 1)
            req->umts[req->cell_count].lac = (uint16_t) (dval);
        else
            err_umts++;
        ...
        uint32_t uval;
        if (p != NULL && p < pe && sscanf(p, XML_TAG_CIS, &uval) == 1)
            req->umts[req->cell_count].ci = uval;
        else
            err_umts++;
        ...
        if (p != NULL && p < pe && sscanf(p, XML_TAG_RSSIS, &dval) == 1) {
            ...
            req->umts[req->cell_count].rssi = (int8_t) (dval);
        } else
            err_umts++;
...
int32_t sky_decode_req_xml(char *buff, int32_t buff_len, int32_t data_len,
    ...
    /* AP */
    if ((count = countTag(buff_s, XML_TAG_AP)) > 0) {
        err_ap = api_req_decode_ap(count, slen, req, buff);
    }

    /* CELL */
    if ((count = countTag(buff_s, XML_TAG_GSM)) > 0) {
        p = api_req_decode_gsm(count, req, buff, err_gsm);
    } else if ((count = countTag(buff_s, XML_TAG_CDMA)) > 0) {
        p = api_req_decode_cdma(count, req, buff, err_cdma);
    } else if ((count = countTag(buff_s, XML_TAG_UMTS)) > 0) {
        p = api_req_decode_umts(count, req, buff, err_umts);
    } else if ((count = countTag(buff_s, XML_TAG_LTE)) > 0) {
        p = api_req_decode_lte(count, req, buff, err_lte);
    }

    /* GPS */
    if ((count = countTag(buff_s, XML_TAG_GPS)) > 0) {
        err_gps = api_req_decode_gps(count, req, buff);
    }

    /* BLE */
    if ((count = countTag(buff_s, XML_TAG_BLE)) > 0) {
